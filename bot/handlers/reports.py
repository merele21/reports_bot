...
async def handle_photo_message(message: Message, session: AsyncSession):
    ...
    channel = await ChannelCRUD.get_by_telegram_id(session, message.chat.id)

    if not channel or not channel.is_active:
        return  # Канал не зарегистрирован или неактивен
    ...
    if message.media_group_id:
        ...
        # Ждем все фото из группы (простая логика с таймаутом)
        # В продакшене лучше использовать FSM или отложенную обработку
            ...
            if photos_count >= channel.min_photos:
                ...
                if is_valid:
                    ...
                    logger.info(
                        ...
                        f"channel={channel.title}, photos={photos_count}"
                    )
                else:
        ...
        if photos_count >= channel.min_photos and is_valid:
            ...
            await message.reply("❌ Ошибки в отчете:\n" + "\n".join(f"• {err}" for err in errors))